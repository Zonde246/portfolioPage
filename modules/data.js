...
export const PROJECTS = [
  ...
  },
  {
    ...
    dossier: {
      ...
      outcome: '88% accuracy on family-disjoint held-out evaluation (TheZoo vs VirusShare) with 11% FNR — designed to test generalization to unseen malware families. 99% accuracy / 0.9971 ROC-AUC on random split for comparison.||Accepted at Scientific Reports; in press.',
  ...
  },
  {
    ...
    dossier: {
      ...
      outcome: 'The gap is documented rather than assumed. A 2025 paper states plainly that accurate vehicle-specific delay information is infeasible to obtain under the heterogeneous, lane-less conditions found in countries like India. It then sidesteps the problem, using scalar queue length on homogeneous traffic. A 2024/25 review of the field does not treat lane-less traffic as a focus at all.||Track A builds and passes smoke tests on both toolchains. Track B implementation follows the selected angle.',
  ...
  },
  {
    ...
    dossier: {
      ...
      contribution: 'Two-person team with Suhani Bahteja. We built the system jointly rather than splitting it into owned halves, so the work below is shared.||The collection, cleaning, annotation and validation pipeline end to end, and the summarisation and evaluation stack above it.||A Unicode-aware tokeniser for Devanagari, so ROUGE and BLEU score Hindi correctly instead of silently mis-segmenting it.||A stratified 100-record audit of gold-summary quality, with target quality measured per outlet, so the corpus can be selected on rather than trusted wholesale.',
